...
import { listRoleMenuIds, updateRoleMenu } from '@/service/system/role'
import { nextTick, onMounted, reactive, ref, toRefs, watch } from 'vue'
import { ElTree, ElMessage, ElMessageBox } from 'element-plus'
import { Switch, Check } from '@element-plus/icons-vue'
import { listTreeSelectMenus } from '@/service/system/menu'
...
watch(
  ...
  (newVal, oldVal) => {
    ...
    if (roleId) {
      ...
      listRoleMenuIds(roleId).then((response: any) => {
...
async function loadTreeSelectMenuOptions() {
  await listTreeSelectMenus().then((response: any) => {
